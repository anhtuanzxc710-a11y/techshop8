...
const createNotification = async(req,res)=>{
    try {
        ...
        const newData = await notificationModel.create({
            userId,
            text,
            isRead: false,
        });
...
const getNewNotification = async (req, res) => {
    try {
      ...
      return res.status(200).json({success:true, data: notifications.slice(0, 10)});
    } catch (error) {
  ...
  const getNotificationsByUser = async (req, res) => {
    try {
        ...
        const notifications = await notificationModel.find({ userId });
        return res.status(200).json({ success: true, data: notifications.slice(0, 15) });
    } catch (error) {
        console.error('Error getting notifications:', error);
